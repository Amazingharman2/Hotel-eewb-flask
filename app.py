...
# NOTE: Because this uses global memory, it will NOT work with multiple workers (Gunicorn).
# It must run in a single-threaded/worker environment (which the setup below ensures).
running_procs = {}
start_times = {}

# Initialize SocketIO with threading mode to support subprocess management without Redis
socketio = SocketIO(async_mode='threading', cors_allowed_origins="*", logger=False, engineio_logger=False)

def get_db():
    # Ensure the storage directory exists before getting DB
...
def create_app():
    app = Flask(__name__)
    app.config['SECRET_KEY'] = 'nehost_ultra_pro_max_99' # In production, set this as an Environment Variable
    
    # Determine base paths (works on Render and local)
    base_dir = os.getcwd()
    app.config['BASE_STORAGE'] = os.path.join(base_dir, 'storage/instances')
    ...
    def server_action(folder, act):
        ...
        if act == 'install':
            ...
            if os.path.exists(req_path):
                ...
                f_log.flush()
                # Use pip3 explicitly
                subprocess.Popen(['pip3', 'install', '-r', 'requirements.txt'], cwd=path, stdout=f_log, stderr=f_log)
                db.close()
        ...
        if act in ['start', 'restart']:
            ...
            # Use python3 explicitly for Render
...
if __name__ == "__main__":
    ...
    # Run the app
    # Note: We use 'threading' mode. Do not use Gunicorn multi-worker 
    # because this script relies on global process tracking.
